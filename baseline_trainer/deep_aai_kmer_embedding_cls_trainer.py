...
class Trainer(object):
    ...
    def train(self, display=True):
        for epoch in range(1, self.param_dict['epoch_num'] + 1):
            ...
            # if epoch % 100 == 0:
            #     plot_loss_curve_v10(epoch, self.train_loss_list, self.seen_valid_loss_list, self.seen_test_loss_list, self.unseen_test_loss_list,
            #                     self.train_acc_list, self.seen_valid_acc_list, self.seen_test_acc_list, self.unseen_test_acc_list, self.untest_auc_list, save_path=self.save_img_path)
